...
const courseProjects = {
  chakra: [
    { title: "Line Following Robot", description: "Build a robot that follows paths", week: 3, parentOutcome: "Develops logical thinking and problem-solving skills" },
    { title: "Obstacle Avoidance Bot", description: "Create a smart navigation system", week: 5, parentOutcome: "Builds confidence in robotics and programming" },
    { title: "Sumo Robot", description: "Compete in robot battles", week: 7, parentOutcome: "Encourages creativity and competitive spirit" },
  ],
};
...
const courseTestimonials = {
  chakra: [
    { name: "Rohan, Age 10", text: "My robot can follow lines and avoid obstacles. So cool! 🤖", rating: 5, isParent: false },
    { name: "Isha, Age 9", text: "I built my first robot and it actually works!", rating: 5, isParent: false },
    { name: "Priya M.", text: "My daughter was so proud when her robot worked! The course structure is perfect for kids.", rating: 5, isParent: true, childAge: "Age 9" },
  ],
};
...
export default function ChakraPage() {
  ...
  const projects = courseProjects.chakra;
  const kitItems = kitContents.chakra;
  const testimonials = courseTestimonials.chakra;
  const faqs = courseFAQs.chakra;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {course.learningOutcomes.map((outcome, index) => {
                const futureCourseLinks: Record<number, { name: string; color: string }> = {
                  0: { name: "YANTRA (IoT)", color: "rgb(168, 85, 247)" },
                  1: { name: "YANTRA (IoT)", color: "rgb(168, 85, 247)" },
                  2: { name: "ANANTA (AI)", color: "rgb(236, 72, 153)" },
                  3: { name: "GARUDA (Drones)", color: "rgb(34, 211, 238)" },
                };
