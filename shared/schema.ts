...
export const testimonialsBM1 = pgTable("testimonialsBM1", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: text("name").notNull(),
  role: text("role").notNull(),
  content: text("content").notNull(),
...
export const insertTestimonialSchemaBM1 = createInsertSchema(testimonialsBM1).omit({
  id: true,
});

export type InsertTestimonialBM1 = z.infer<typeof insertTestimonialSchemaBM1>;
export type TestimonialBM1 = typeof testimonialsBM1.$inferSelect;

// STEMKit type for product kits
export type STEMKit = {
  id: string;
  name: string;
  subtitle: string;
